			       ...
			       addr16_t address, pagelen_t len)
{
	 ...
	 */
	{
      ...
      uint8_t * wPtr = (uint8_t*)mybuff.bptr;
      uint16_t length;
      for(length = 0; length < SPM_PAGESIZE; length+=4, wPtr+=4) {
        EEARL = 0; EEDR = wPtr[0];
        EEARL = 1; EEDR = wPtr[1];
        EEARL = 2; EEDR = wPtr[2];
        EEARL = 3; EEDR = wPtr[3];
        EEARH = (addrPtr + length)>>8;
        EEARL = (addrPtr + length)&0xff;
        EECR = 0xA4; // 0b1110 (flash program) 0100 (inturrept, enable program ctl, enable program, read)
        EECR = 0xA2; // 0b1110 (flash program) 0010 (inturrept, enable program ctl, enable program, read)
      }
#else
	    /*
